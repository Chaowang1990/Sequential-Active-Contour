...
for elem=1:length(non_zero)
   cent_ind = non_zero(elem);
    [ Iadj , Radj, Nfound] =  neighbourND( cent_ind, sss);
    neighb_ind = Iadj;
    phi_neigh = phi(neighb_ind);
    center_phi =phi(cent_ind);
    clique=ones(1 , 9);
        clique(5)=0;
      sig_neigh= sqrt(var(Img(neighb_ind),0,"all"));
     for k= 1:length( neighb_ind)
            ind_temp = lin_in(k);
            if sign(center_phi)==sign(phi_neigh(ind_temp))
                clique(ind_temp)=-exp(-abs(Img(cent_ind)-Img(ind_temp)));
            else 
                 clique(ind_temp)=(2/(1+exp(-abs(Img(cent_ind)-Img(ind_temp)))));
            end
            
     end
        prior(elem)=sum(clique) + log(2*pi)*2*sig_neigh;
end
