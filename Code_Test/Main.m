...
cd '\\iowa.uiowa.edu\shared\engineering\home\afallahdizcheh\windowsdata\Desktop\Code_Test';
...
for iter = 2:NumIter
    [phi lf]=evolution_LGDF_1(Img,phi,epsilon,Ksigma,KONE,KI,KI2,mu,nu,lambda1,lambda2,timestep,alf,lf);
    %if(mod(iter,1) == 0)
       % figure(2),
...
%tic
   [phi lf]=evolution_proposed_initial(Img,phi,epsilon,Ksigma,KONE,KI,KI2,mu,nu,lambda1,lambda2,timestep,alf);
for iter = 2:NumIter
    [phi lf]=evolution_LSACM(Img,phi,epsilon,Ksigma,KONE,KI,KI2,mu,nu,lambda1,lambda2,timestep,alf,lf);
    %if(mod(iter,1) == 0)
       % figure(2),
       % imagesc(uint8(Img),[0 255]),colormap(gray),axis off;axis equal,title(num2str(iter))
       % hold on,[c,h] = contour(phi,[0 0],'r','linewidth',1); hold off
...
%tic
   [phi lf]=evolution_proposed_initial(Img,phi,epsilon,Ksigma,KONE,KI,KI2,mu,nu,lambda1,lambda2,timestep,alf);
for iter = 2:NumIter
    [phi lf]=evolution_Bayes_1(Img,phi,epsilon,Ksigma,KONE,KI,KI2,mu,nu,lambda1,lambda2,timestep,alf,lf);
    %if(mod(iter,1) == 0)
       % figure(2),
       % imagesc(uint8(Img),[0 255]),colormap(gray),axis off;axis equal,title(num2str(iter))
       % hold on,[c,h] = contour(phi,[0 0],'r','linewidth',1); hold off
...
%tic
   [phi lf]=evolution_proposed_initial(Img,phi,epsilon,Ksigma,KONE,KI,KI2,mu,nu,lambda1,lambda2,timestep,alf);
for iter = 2:NumIter
    [phi lf]=evolution_truth(Img,phi,epsilon,Ksigma,KONE,KI,KI2,mu,nu,lambda1,lambda2,timestep,alf,lf);
    %if(mod(iter,1) == 0)
       % figure(2),
       % imagesc(uint8(Img),[0 255]),colormap(gray),axis off;axis equal,title(num2str(iter))
       % hold on,[c,h] = contour(phi,[0 0],'r','linewidth',1); hold off
...
%tic
   [phi lf]=evolution_proposed_initial(Img,phi,epsilon,Ksigma,KONE,KI,KI2,mu,nu,lambda1,lambda2,timestep,alf);
for iter = 2:NumIter
    [phi lf]=evolution_UNET(Img,phi,epsilon,Ksigma,KONE,KI,KI2,mu,nu,lambda1,lambda2,timestep,alf,lf);
    %if(mod(iter,1) == 0)
       % figure(2),
       % imagesc(uint8(Img),[0 255]),colormap(gray),axis off;axis equal,title(num2str(iter))
       % hold on,[c,h] = contour(phi,[0 0],'r','linewidth',1); hold off
