...
function [u lf]= evolution_proposed(Img,u,epsilon,Ksigma,KONE,KI,KI2,mu,nu,lambda1,lambda2,timestep,alf,lf_in)
u=NeumannBound(u);
...
sigma1 = sigma1 + eps;
sigma2 = sigma2 + eps;
size_force = size(lf_in);
prior =zeros(size_force);
mat_max = Img > 100 & Img <200;
non_zero = find(mat_max);
for elem=1:length(non_zero)
   cent_ind = non_zero(elem);
    [ Iadj , Radj, Nfound] =  neighbourND( cent_ind, size_force);
    neighb_ind = Iadj;
    weight_vec = exp(-abs(Img(cent_ind)-Img(neighb_ind))/10);
    prior(cent_ind)=(sum(lf_in(neighb_ind).*weight_vec))/sum(weight_vec);
end
...
A = -alf.*Delta.*(localForce+prior);%data force
